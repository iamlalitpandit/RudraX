 ...
 */
import { modelsAreEqual, supportsXhigh } from "@imlalitpandit/pi-ai";
...
import { ensureFirstRunSetup, runSetupWizard } from "./setup/setup-wizard.js";
...
Options:
  -l, --local    Install project-locally (.rudrax/settings.json)
...
Options:
  -l, --local    Remove from project settings (.rudrax/settings.json)
...
export async function main(args) {
    resetTimings();
    if (args[0] === "setup") {
        await runSetupWizard({ defaults: args.includes("--defaults"), force: args.includes("--force") });
        return;
    }
    const setupSafeCommand = ["--help", "-h", "--version", "-v", "--list-models", "install", "remove", "uninstall", "update", "list", "config"].includes(args[0]);
    if (!setupSafeCommand) await ensureFirstRunSetup();
    const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.RUDRAX_OFFLINE);
    if (offlineMode) {
        process.env.RUDRAX_OFFLINE = "1";
        process.env.RUDRAX_SKIP_VERSION_CHECK = "1";
    }
    ...
    const startupBenchmark = isTruthyEnvFlag(process.env.RUDRAX_STARTUP_BENCHMARK);
    if (startupBenchmark && !isInteractive) {
        console.error(chalk.red("Error: RUDRAX_STARTUP_BENCHMARK only supports interactive mode"));
        process.exit(1);
    }
