...
import { isContextOverflow, modelsAreEqual, resetApiProviders, supportsXhigh } from "@imlalitpandit/pi-ai";
...
export class AgentSession {
    ...
    async _getRequiredRequestAuth(model) {
        // Ollama models don't require API keys - detect by provider or endpoint
        const isOllama = model.provider === "ollama" || model.api?.includes("127.0.0.1:11434");
        if (isOllama) {
            return { apiKey: "ollama", headers: {} };
        }
    ...
    async prompt(text, options) {
        ...
        // Skip auth check for Ollama models
        const isOllama = this.model.provider === "ollama" || this.model.api?.includes("127.0.0.1:11434");
        if (!isOllama && !this._modelRegistry.hasConfiguredAuth(this.model)) {
    ...
    async setModel(model) {
        // Skip auth validation for Ollama models (detected by Ollama endpoint)
        const isOllama = model.provider === "ollama" || model.api?.includes("127.0.0.1:11434");
        if (!isOllama && !this._modelRegistry.hasConfiguredAuth(model)) {
    ...
    async _runAutoCompaction(reason, willRetry) {
        ...
        try {
            ...
            if (willRetry) {
                ...
                // or while the user is mid-task. Always kick the loop so the agent continues.
                setTimeout(() => {
                    this.agent.continue().catch(() => { });
                }, 100);
            }
