 ...
 */
import { modelsAreEqual } from "@imlalitpandit/pi-ai";
...
export async function findInitialModel(options) {
    ...
    const availableModels = await modelRegistry.getAvailable();
    if (availableModels.length > 0) {
        // Prefer authenticated cloud providers when a local endpoint is merely
        // configured but the local server may not be running.
        const cloudModels = availableModels.filter((candidate) => !getProviderDefinition(candidate.provider)?.noAuth);
        const selectionPool = cloudModels.length > 0 ? cloudModels : availableModels;
        // Try to find a default model from known providers
        for (const provider of Object.keys(defaultModelPerProvider)) {
            const defaultId = defaultModelPerProvider[provider];
            const match = selectionPool.find((m) => m.provider === provider && m.id === defaultId);
            if (match) {
                return { model: match, thinkingLevel: DEFAULT_THINKING_LEVEL, fallbackMessage: undefined };
            }
        }
        // If no default found, use first available from the preferred pool.
        return { model: selectionPool[0], thinkingLevel: DEFAULT_THINKING_LEVEL, fallbackMessage: undefined };
    }
    // 5. No model found
