 ...
 */
import { getModels, getProviders, registerApiProvider, resetApiProviders, } from "@imlalitpandit/pi-ai";
import { registerOAuthProvider, resetOAuthProviders } from "@imlalitpandit/pi-ai/oauth";
...
import { PROVIDER_CATALOG, firstEnvironmentValue, getProviderDefinition, normalizeProviderId } from "./provider-catalog.js";
...
export class ModelRegistry {
    ...
    /** Reload model/auth configuration without resetting the process-global API registry. */
    refreshModelsOnly() {
        this.providerRequestConfigs.clear();
        this.modelRequestHeaders.clear();
        this.loadError = undefined;
    ...
    loadModels() {
        ...
        this.bootstrapCatalogModels(builtInModels);
    ...
    loadBuiltInModels(overrides, modelOverrides) {
        return getProviders().flatMap((provider) => {
            ...
            return models.map((m) => {
                ...
                const envBaseUrl = firstEnvironmentValue(definition?.env.baseUrl);
                return envBaseUrl ? { ...model, baseUrl: envBaseUrl } : model;
            });
        });
    }
    /** Add catalog adapters that pi-ai does not ship, preserving all built-ins. */
    bootstrapCatalogModels(models) {
        const seen = new Set(models.map((model) => `${model.provider}\0${model.id}`));
        for (const provider of PROVIDER_CATALOG) {
            const baseUrl = firstEnvironmentValue(provider.env.baseUrl) ?? provider.baseUrl;
            const configuredModel = firstEnvironmentValue(provider.env.model);
            const requestedApiMode = firstEnvironmentValue(provider.env.apiMode)?.toLowerCase();
            const transport = requestedApiMode === "anthropic" || requestedApiMode === "anthropic-messages"
                ? "anthropic-messages"
                : provider.transport;
            const modelIds = configuredModel ? [configuredModel] : provider.models;
            for (const id of modelIds) {
                const key = `${provider.id}\0${id}`;
                if (seen.has(key))
                    continue;
                seen.add(key);
                models.push({
                    id, name: id, provider: provider.id, api: transport, baseUrl,
                    reasoning: false, input: ["text"],
                    cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
                    contextWindow: 128000, maxTokens: 16384, headers: undefined, compat: undefined,
                });
            }
        }
    }
    ...
    /**
     * Register a custom model dynamically (e.g., for Ollama)
     */
    registerCustomModel(model) {
        // Check if model already exists
        const existingIndex = this.models.findIndex(m => m.provider === model.provider && m.id === model.id);
        if (existingIndex >= 0) {
            // Replace existing
    ...
    find(provider, modelId) {
        ...
        return this.models.find((m) => m.provider === canonical && m.id === modelId);
    }
    /**
    ...
    hasConfiguredAuth(model) {
        ...
        if (definition && !model.baseUrl && !firstEnvironmentValue(definition.env.baseUrl)) {
            return false;
        }
        if (definition?.noAuth) {
            // Do not auto-select every catalogued local server merely because it
            // needs no key. It becomes available after an explicit endpoint/model
            // configuration; WebUI keeps Ollama process discovery as a fallback.
            return Boolean(firstEnvironmentValue(definition.env.baseUrl) || this.providerRequestConfigs.has(model.provider));
        }
        if (model.provider === "ollama" || model.api?.includes("127.0.0.1:11434")) {
            return true;
        }
        const providerApiKey = this.providerRequestConfigs.get(model.provider)?.apiKey;
        let resolvedProviderKey = false;
        if (providerApiKey) {
            try {
                resolvedProviderKey = definition?.env.apiKey.includes(providerApiKey)
                    ? Boolean(process.env[providerApiKey])
                    : Boolean(resolveConfigValueUncached(providerApiKey));
            }
            catch {
                resolvedProviderKey = false;
            }
        }
        return this.authStorage.hasAuth(model.provider) || resolvedProviderKey;
    }
    ...
    async getApiKeyAndHeaders(model) {
        try {
            const definition = getProviderDefinition(model.provider);
            const noAuthAllowed = definition?.noAuth || model.provider === "ollama" || model.api?.includes("127.0.0.1:11434");
            const providerConfig = this.providerRequestConfigs.get(model.provider);
            const apiKeyFromAuthStorage = await this.authStorage.getApiKey(model.provider, { includeFallback: false });
            if (noAuthAllowed && !apiKeyFromAuthStorage) {
                return { ok: true, apiKey: "ollama", headers: undefined };
            }
            ...
            if (apiKey && definition?.authHeader === "api-key")
                // OpenAI SDK adds Bearer auth by default; null explicitly removes it
                // so Azure Foundry receives only its required api-key header.
                headers = { ...headers, Authorization: null, "api-key": apiKey };
            if (apiKey && definition?.authHeader === "x-api-key")
    ...
    validateProviderConfig(providerName, config) {
        ...
        if (!config.apiKey && !config.oauth && !getProviderDefinition(providerName)?.noAuth) {
