...
function getBuiltinThemes() {
    if (!BUILTIN_THEMES) {
        ...
        const rudraxPath = path.join(themesDir, "rudrax.json");
        BUILTIN_THEMES = {
            dark: JSON.parse(fs.readFileSync(darkPath, "utf-8")),
            light: JSON.parse(fs.readFileSync(lightPath, "utf-8")),
            rudrax: JSON.parse(fs.readFileSync(rudraxPath, "utf-8")),
        };
    }
...
function detectTerminalBackground() {
    // RudraX always uses its custom theme
    return "rudrax";
}
...
const THEME_KEY = Symbol.for("@imlalitpandit/pi-coding-agent:theme");
...
export function initTheme(themeName, enableWatcher = false) {
    ...
    try {
        ...
        // Theme is invalid - fall back to rudrax theme silently
        currentThemeName = "rudrax";
        setGlobalTheme(loadTheme("rudrax"));
...
export function setTheme(name, enableWatcher = false) {
    ...
    try {
        ...
        // Theme is invalid - fall back to rudrax theme
        currentThemeName = "rudrax";
        setGlobalTheme(loadTheme("rudrax"));
        // Don't start watcher for fallback theme
        return {
