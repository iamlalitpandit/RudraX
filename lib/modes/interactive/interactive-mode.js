...
import { CombinedAutocompleteProvider, Container, fuzzyFilter, Loader, Markdown, matchesKey, ProcessTerminal, Spacer, setKeybindings, Text, TruncatedText, TUI, visibleWidth, } from "@imlalitpandit/pi-tui";
...
import { getOllamaModels, isOllamaAvailable, ensureOllamaRunning, getOllamaModelConfig } from "../../core/ollama-openai-provider.js";
import { autoInitOllamaModel, autoInitGeminiModel } from "../../core/auto-init.js";
...
export class InteractiveMode {
    ...
    async init() {
        ...
        if (this.options.verbose || !this.settingsManager.getQuietStartup()) {
            // Get terminal width for full-width banner
            const termWidth = this.ui.terminal?.width || 80;
            
            // Full-width ASCII Banner - uses entire terminal width
            const lineWidth = termWidth - 2; // 2 for border characters
            const innerWidth = lineWidth - 2; // Inside the borders
            
            // Larger, more impressive RudraX banner that fills the terminal
            const bannerLines = [];
            
            // Top border
            bannerLines.push(" ".repeat(1) + "╔" + "═".repeat(innerWidth) + "╗");
            
            // Big R letter and title - extra large
            const title1 = theme.fg("accent", theme.bold("██████╗ ██╗   ██╗██████╗ ██████╗  █████╗ ██╗  ██╗"));
            const spaces1 = " ".repeat(Math.max(0, innerWidth - title1.replace(/\x1b\[[0-9;]*m/g, "").length));
            bannerLines.push(" ║" + title1 + spaces1 + "║");
            
            const title2 = theme.fg("accent", theme.bold("██╔══██╗██║   ██║██╔══██╗██╔══██╗██╔══██╗╚██╗██╔╝"));
            const spaces2 = " ".repeat(Math.max(0, innerWidth - title2.replace(/\x1b\[[0-9;]*m/g, "").length));
            bannerLines.push(" ║" + title2 + spaces2 + "║");
            
            const title3 = theme.fg("accent", theme.bold("██████╔╝██║   ██║██║  ██║██████╔╝███████║ ╚███╔╝"));
            const spaces3 = " ".repeat(Math.max(0, innerWidth - title3.replace(/\x1b\[[0-9;]*m/g, "").length));
            bannerLines.push(" ║" + title3 + spaces3 + "║");
            
            const title4 = theme.fg("error", theme.bold("██╔══██╗██║   ██║██║  ██║██╔══██╗██╔══██║ ██╔██╗"));
            const spaces4 = " ".repeat(Math.max(0, innerWidth - title4.replace(/\x1b\[[0-9;]*m/g, "").length));
            bannerLines.push(" ║" + title4 + spaces4 + "║");
            
            const title5 = theme.fg("error", theme.bold("██║  ██║╚██████╔╝██████╔╝██║  ██║██║  ██║██╔╝ ██╗"));
            const spaces5 = " ".repeat(Math.max(0, innerWidth - title5.replace(/\x1b\[[0-9;]*m/g, "").length));
            bannerLines.push(" ║" + title5 + spaces5 + "║");
            
            const title6 = theme.fg("error", theme.bold("╚═╝  ╚═╝ ╚═════╝ ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝"));
            const spaces6 = " ".repeat(Math.max(0, innerWidth - title6.replace(/\x1b\[[0-9;]*m/g, "").length));
            bannerLines.push(" ║" + title6 + spaces6 + "║");
            
            // Slogan - centered
            const slogan = theme.fg("success", theme.bold("▶ BUILD  •  BREAK  •  DEPLOY ◀"));
            const sloganPad = " ".repeat(Math.max(0, Math.floor((innerWidth - slogan.replace(/\x1b\[[0-9;]*m/g, "").length) / 2)));
            bannerLines.push(" ║" + sloganPad + slogan + " ".repeat(innerWidth - sloganPad.length - slogan.replace(/\x1b\[[0-9;]*m/g, "").length) + "║");
            
            // Version and credits
            const versionLine = theme.fg("dim", `v${this.version}`) + theme.fg("muted", " • ") + theme.fg("dim", theme.bold("by Lalit Pandit"));
            const verPad = " ".repeat(Math.max(0, Math.floor((innerWidth - versionLine.replace(/\x1b\[[0-9;]*m/g, "").length) / 2)));
            bannerLines.push(" ║" + verPad + versionLine + " ".repeat(innerWidth - verPad.length - versionLine.replace(/\x1b\[[0-9;]*m/g, "").length) + "║");
            
            // Bottom border
            bannerLines.push(" ╚" + "═".repeat(innerWidth) + "╝");
            ...
            const welcomeText = [
                "",
                " ".repeat(Math.max(0, Math.floor((termWidth - 20) / 2))) + theme.fg("accent", theme.bold("👋 Welcome to RudraX")),
                "",
                " ".repeat(Math.max(0, Math.floor((termWidth - 28) / 2))) + theme.fg("text", "How can I help you today?"),
                "",
                " ".repeat(Math.max(0, Math.floor((termWidth - 44) / 2))) + theme.fg("dim", "Type your message or drag files to attach."),
                " ".repeat(Math.max(0, Math.floor((termWidth - 66) / 2))) + theme.fg("dim", "Use /help for commands, /ollama or /gemini to select models."),
                "",
            ].join("\n");
            
            const fullHeader = `${banner}\n${welcomeText}`;
            
            this.builtInHeader = new Text(fullHeader, 1, 0);
            // Setup UI layout - cleaner without changelog
            this.headerContainer.addChild(new Spacer(1));
            this.headerContainer.addChild(this.builtInHeader);
            this.headerContainer.addChild(new Spacer(1));
            // Skip changelog display for cleaner look
        }
        else {
    ...
    updateTerminalTitle() {
        ...
        if (sessionName) {
            this.ui.terminal.setTitle(`RudraX - ${sessionName} - ${cwdBasename}`);
        }
        else {
            this.ui.terminal.setTitle(`RudraX - ${cwdBasename}`);
        }
    }
    /**
    ...
    async run() {
        await this.init();
        // Skip version checks and warnings for clean RudraX experience
        // Check tmux keyboard setup asynchronously (keep this)
        this.checkTmuxKeyboardSetup().then((warning) => {
            if (warning) {
                this.showWarning(warning);
            }
        });
        // Auto-start WebUI server in background
        try {
            const { autoStartWebUI } = await import('../../core/webui-manager.js');
            const webuiResult = await autoStartWebUI();
            if (webuiResult) {
                this.showStatus(`🔥 WebUI: ${webuiResult.url || webuiResult.message}`);
            }
        } catch (e) {
            // WebUI auto-start is best-effort; don't block TUI startup
        }
        // Auto-initialize models if none selected
        if (!this.session.model) {
            console.log("No model selected, auto-initializing...");
            
            // First try Ollama (local models)
            const ollamaInitialized = await autoInitOllamaModel(this.session, this.footer, () => this.updateEditorBorderColor(), (msg) => this.showStatus(msg));
            
            // If Ollama wasn't available, try Gemini
            if (!ollamaInitialized) {
                console.log("Ollama not available, trying Gemini...");
                await autoInitGeminiModel(this.session, this.footer, () => this.updateEditorBorderColor(), (msg) => this.showStatus(msg));
            }
        }
        ...
        // Skip model.json errors - Ollama mode doesn't require it
        ...
        while (true) {
            ...
            console.error(`[DEBUG] Input received: "${userInput}"`);
            try {
                console.error(`[DEBUG] Model before prompt:`, this.session.model?.id || "none");
                await this.session.prompt(userInput);
                console.error(`[DEBUG] Prompt completed`);
            }
            catch (error) {
                console.error(`[DEBUG] Prompt error:`, error);
                // Ensure loader is stopped on error to fix stuck "Working..." issue
    ...
    async checkForNewVersion() {
        if (process.env.RUDRAX_SKIP_VERSION_CHECK || process.env.RUDRAX_OFFLINE)
            return undefined;
        try {
            const response = await fetch("https://registry.npmjs.org/@imlalitpandit/pi-coding-agent/latest", {
    ...
    async checkTmuxKeyboardSetup() {
        ...
        if (extendedKeysFormat === "xterm") {
            return "tmux extended-keys-format is xterm. RudraX works best with csi-u. Add `set -g extended-keys-format csi-u` to ~/.tmux.conf and restart tmux.";
        }
        return undefined;
    }
    /**
    ...
    setupEditorSubmitHandler() {
        this.defaultEditor.onSubmit = async (text) => {
            ...
            if (text === "/model" || text.startsWith("/model ")) {
                this.editor.setText("");
                // Show all available models (including custom providers)
                const searchTerm = text.startsWith("/model ") ? text.slice(7).trim() : undefined;
                await this.handleModelCommand(searchTerm);
                return;
            }
            // /ollama is now alias for /model
            if (text === "/gemini" || text.startsWith("/gemini ")) {
                this.editor.setText("");
                await this.handleGeminiCommand();
                return;
            }
            ...
            // Removed: /login and /logout - no authentication needed for Ollama
            if (text === "/new") {
                this.editor.setText("");
    ...
    showNewVersionNotification(newVersion) {
        const action = theme.fg("accent", getUpdateInstruction("@imlalitpandit/pi-coding-agent"));
    ...
    async handleOllamaCommand() {
        // Check if Ollama is available
        const isAvailable = await isOllamaAvailable();
        if (!isAvailable) {
            this.showError("Ollama not found. Please install Ollama: https://ollama.com");
            return;
        }
        // Check if server is running
        this.showStatus("Checking Ollama server...");
        const isRunning = await ensureOllamaRunning();
        if (!isRunning) {
            this.showError("Ollama server not running. Start it with: ollama serve");
            return;
        }
        // Get models
        this.showStatus("Fetching Ollama models...");
        try {
            const models = await getOllamaModels();
            if (models.length === 0) {
                this.showError("No Ollama models found. Install with: ollama pull <model>");
                return;
            }
            // Show model selector
            await this.showOllamaModelSelector(models);
        }
        catch (error) {
            this.showError(`Failed to get Ollama models: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async showOllamaModelSelector(ollamaModels) {
        this.showSelector((done) => {
            const items = ollamaModels.map((m, i) => ({
                id: m.id,
            ...
            selector.addChild(new Spacer(1));
            // Add header
            const header = new Text(theme.bold(theme.fg("accent", "🦙 Select Ollama Model")), 1, 0);
            selector.addChild(header);
            selector.addChild(new Text(theme.fg("dim", `${ollamaModels.length} models available`), 1, 0));
            selector.addChild(new Spacer(1));
            // Add models
            let selectedIndex = 0;
            function updateList() {
                // Clear previous list
                while (selector.children.length > 4) {
                ...
                items.forEach((item, i) => {
                    ...
                    const text = new Text(theme.fg(color, `${prefix}${item.text}`), 1, 0);
                    selector.addChild(text);
                });
                selector.addChild(new Spacer(1));
                selector.addChild(new Text(theme.fg("dim", "↑/↓ to navigate, Enter to select, Esc to cancel"), 1, 0));
                selector.addChild(new DynamicBorder());
            }
            updateList();
            // Handle input
            const handleInput = (data) => {
                if (data === "\x1b[A" || data === "k") { // Up arrow or k
                ...
                else if (data === "\x1b[B" || data === "j") { // Down arrow or j
                    ...
                    if (selected) {
                        const modelConfig = getOllamaModelConfig(selected.model.id);
                        // Register the model before setting it
                        this.session.modelRegistry.registerCustomModel(modelConfig);
                        this.session.setModel(modelConfig).then(() => {
                            this.footer.invalidate();
                            this.updateEditorBorderColor();
                            this.showStatus(`Model: ${modelConfig.id}`);
                            done();
                        }).catch((err) => {
            ...
            return { component: selector, focus: { handleInput } };
        });
    }
    async handleGeminiCommand() {
        // Check if Gemini API key is available
        const { isGeminiAvailable, getGeminiModels, getGeminiModelConfig } = await import("../../core/gemini-provider.js");
        
        if (!isGeminiAvailable()) {
            this.showError("Gemini not configured. Set GEMINI_API_KEY environment variable or add it to auth.json");
            return;
        }
        
        // Get models
        this.showStatus("Loading Gemini models...");
        try {
            const models = getGeminiModels();
            if (models.length === 0) {
                this.showError("No Gemini models found in registry");
                return;
            }
            // Show model selector
            await this.showGeminiModelSelector(models);
        }
        catch (error) {
            this.showError(`Failed to get Gemini models: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async showGeminiModelSelector(geminiModels) {
        this.showSelector((done) => {
            const items = geminiModels.map((m, i) => ({
                id: m.id,
                text: `${i + 1}. ${m.name}${m.reasoning ? " [thinking]" : ""}`,
                model: m
            }));
            const selector = new Container();
            // Add border
            ...
            selector.addChild(new Spacer(1));
            // Add header
            const header = new Text(theme.bold(theme.fg("accent", "✨ Select Gemini Model")), 1, 0);
            selector.addChild(header);
            selector.addChild(new Text(theme.fg("dim", `${geminiModels.length} models available`), 1, 0));
            selector.addChild(new Spacer(1));
            // Add models
            let selectedIndex = 0;
            function updateList() {
            ...
            const handleInput = (data) => {
                if (data === "\x1b[A" || data === "k") { // Up arrow or k
                    ...
                    const selected = items[selectedIndex];
                    if (selected) {
                        // Register model before setting it
