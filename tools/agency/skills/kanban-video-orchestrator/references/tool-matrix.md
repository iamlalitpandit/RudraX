...
Maps each role archetype to the RudraX skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public rudrax-agent
...
## RudraX skills relevant to video production

### Visual / rendering skills (`rudrax-agent/skills/creative/`)
...
### Design / pre-production skills (`rudrax-agent/skills/creative/`)
...
### Audio / media skills (`rudrax-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`rudrax-agent/skills/devops/`)
...
These are **not** RudraX skills but external CLIs / APIs that profiles invoke.
...
## Built-in RudraX tools for media review

These are native RudraX tools — not invoked via terminal but through their own
...
key is present in `~/.rudrax/agent/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every RudraX profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.rudrax/agent/.env` → environment variable.
...
When a new RudraX-public video skill ships:
