...
rudrax kanban watch --tenant <project-slug>

# Snapshot of the board
rudrax kanban list --tenant <project-slug>
rudrax kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
rudrax kanban stats --tenant <project-slug>

# Visual dashboard (browser)
rudrax dashboard

# Inspect a specific task (includes comments + events)
rudrax kanban show <task-id>

# Follow a single task's event stream
rudrax kanban tail <task-id>
```

Verify available subcommands with `rudrax kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `rudrax kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `rudrax kanban block <id>` then `rudrax kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `rudrax kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `rudrax kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `rudrax profile list` (profile exists?), `rudrax status` (gateway/dispatcher up?), and verify tenant. |
...
rudrax kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
rudrax kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(rudrax kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
rudrax kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
rudrax kanban block <task-id>
rudrax kanban archive <task-id>

# Diagnose what happened
rudrax kanban show <task-id>      # task body, comments, recent events
rudrax kanban tail <task-id>      # follow the live event stream
rudrax kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `rudrax kanban create` for the director
...
while true; do
    clear
    rudrax kanban list --tenant <slug>
    echo "---"
    rudrax kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `rudrax kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `rudrax profile list` and start
