#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for RudraX.
...
# Ensure sibling modules (_rudrax_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _rudrax_home import display_rudrax_home, get_rudrax_home

RUDRAX_CODING_AGENT_DIR = get_rudrax_home()
TOKEN_PATH = RUDRAX_CODING_AGENT_DIR / "google_token.json"
CLIENT_SECRET_PATH = RUDRAX_CODING_AGENT_DIR / "google_client_secret.json"
PENDING_AUTH_PATH = RUDRAX_CODING_AGENT_DIR / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same RudraX profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'rudrax-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to RudraX home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_rudrax_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for RudraX")
