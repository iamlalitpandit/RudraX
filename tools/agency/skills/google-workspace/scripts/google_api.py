#!/usr/bin/env python3
"""Google Workspace API CLI for RudraX.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing RudraX-facing JSON contract and falls back to the Python client
...
from _rudrax_home import get_rudrax_home
...
def _gws_binary() -> str | None:
    override = os.getenv("RUDRAX_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for RudraX")
