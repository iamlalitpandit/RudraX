...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("RUDRAX_CODING_AGENT_DIR", os.path.expanduser("~/.rudrax/agent")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("RUDRAX_CODING_AGENT_DIR", Path.home() / ".rudrax")) / "skills" / "red-teaming" / "godmode"
...
RUDRAX_CODING_AGENT_DIR = Path(os.getenv("RUDRAX_CODING_AGENT_DIR", Path.home() / ".rudrax"))
CONFIG_PATH = RUDRAX_CODING_AGENT_DIR / "config.yaml"
PREFILL_PATH = RUDRAX_CODING_AGENT_DIR / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Explicitly uncensored models, just needs clean prompt
    "rudrax": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "rudrax" in model_lower or "nous" in model_lower:
        return "rudrax"
...
def _get_current_model() -> tuple:
    """Read current model and provider from RudraX config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.rudrax/agent/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart RudraX for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart RudraX for changes to take effect.")
