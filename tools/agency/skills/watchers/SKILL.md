...
All three live in `$RUDRAX_CODING_AGENT_DIR/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$RUDRAX_CODING_AGENT_DIR/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $RUDRAX_CODING_AGENT_DIR/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.rudrax/agent/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $RUDRAX_CODING_AGENT_DIR/skills/devops/watchers/scripts/watch_github.py \
  --name rudrax-issues --repo iamlalitpandit/RudraX --scope issues
```

Poll an arbitrary JSON API:

```bash
python $RUDRAX_CODING_AGENT_DIR/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$RUDRAX_CODING_AGENT_DIR/watcher-state/<name>.json`. Inspect:

```bash
cat $RUDRAX_CODING_AGENT_DIR/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $RUDRAX_CODING_AGENT_DIR/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$RUDRAX_CODING_AGENT_DIR/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
