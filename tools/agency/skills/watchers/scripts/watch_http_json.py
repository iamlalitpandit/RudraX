...
Usage (via cron with --no-agent):
    ...
    rudrax cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$RUDRAX_CODING_AGENT_DIR/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "RudraX-Watcher/1.0"})
