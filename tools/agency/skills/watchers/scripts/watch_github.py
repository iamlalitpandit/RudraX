...
Usage (via cron with --no-agent):
    ...
    rudrax cron create rudrax-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$RUDRAX_CODING_AGENT_DIR/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name rudrax-issues --repo iamlalitpandit/RudraX --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.rudrax/agent/.env to avoid the 60 req/hr
