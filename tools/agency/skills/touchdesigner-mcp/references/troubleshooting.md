...
## 5. RudraX Configuration

### Config location

`$RUDRAX_CODING_AGENT_DIR/config.yaml` (defaults to `~/.rudrax/agent/config.yaml` when `RUDRAX_CODING_AGENT_DIR` is unset)
...
Restart the RudraX session for changes to take effect. The MCP connection is
