...
metadata:
  ...
  original_name: "touchdesigner-mcp"
---
...
```
RudraX -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${RUDRAX_CODING_AGENT_DIR:-$HOME/.rudrax}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to RudraX config (if missing)
...
3. **Restart RudraX session** to pick up the new MCP server
