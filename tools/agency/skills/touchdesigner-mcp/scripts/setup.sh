...
RUDRAX_CODING_AGENT_DIR_DIR="${RUDRAX_CODING_AGENT_DIR:-$HOME/.rudrax}"
RUDRAX_CFG="${RUDRAX_CODING_AGENT_DIR_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure RudraX config has twozero_td MCP entry ──
if [[ ! -f "$RUDRAX_CFG" ]]; then
    echo -e " ${FAIL} RudraX config not found at ${RUDRAX_CFG}"
    manual_steps+=("Create ${RUDRAX_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$RUDRAX_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in RudraX config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to RudraX config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${RUDRAX_CFG} manually"); }
    manual_steps+=("Restart RudraX session to pick up config change")
fi
